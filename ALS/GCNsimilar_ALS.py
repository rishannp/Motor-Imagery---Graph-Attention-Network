    ...
    for fold, (train_idx, test_idx) in enumerate(kf.split(data_list)):
        ...
        class GCN(nn.Module):
            def __init__(self, hidden_channels):
                ...
                # Define GCN convolution layers
                self.conv1 = GCNConv(8, hidden_channels)  # Input to hidden layer
                ...
                self.lin = nn.Linear(hidden_channels, 2)  # Number of classes
        
            def forward(self, x, edge_index, batch):
                # Apply first GCN layer and normalization
                x = self.conv1(x, edge_index)
                x = F.relu(x)
                x = self.gn1(x, batch)  # Apply GraphNorm
        
                # Apply second GCN layer and normalization
                x = self.conv2(x, edge_index)
                x = F.relu(x)
                x = self.gn2(x, batch)  # Apply GraphNorm
        
                # Apply third GCN layer and normalization
